...
TEST(test_block_allocator, test_allocator_alloc_blocks_are_aligned)
{
	std::array<std::byte, 1_kB> buffer = {};
	wmcv::Block mem{.address = wmcv::ptr_to_address(buffer.data()), .size = buffer.size()};
	wmcv::BlockAllocator pool(mem, 64, 8);
...
TEST(test_block_allocator, test_allocator_alloc_not_enough_space)
{
	std::array<std::byte, 4_kB> buffer = {};
	wmcv::Block mem{.address = wmcv::ptr_to_address(buffer.data()), .size = buffer.size()};
	wmcv::BlockAllocator pool(mem, 4_kB, 4);
...
TEST(test_block_allocator, test_allocator_clear)
{
	std::array<std::byte, 4_kB> buffer = {};
	wmcv::Block mem{.address = wmcv::ptr_to_address(buffer.data()), .size = buffer.size()};
	wmcv::BlockAllocator pool(mem, 4_kB, 4);
